...
picoos_uint8 picobase_get_next_utf8char(const picoos_uint8 *utf8s,
    ...
    /* 64-bit adaptation: enhanced input validation */
    if (utf8s == NULL || pos == NULL || utf8char == NULL) {
        if (utf8char != NULL) utf8char[0] = 0;
        return FALSE;
    }
    
    /* 64-bit adaptation: check pointer alignment */
    #ifdef PICO_64BIT_SUPPORT
    if (((picoos_ptr_t)utf8s & 0x3) != 0) {
        /* Pointer not aligned, may have issues on 64-bit systems */
    ...
    /* 64-bit adaptation: enhanced length checking */
    ...
    /* 64-bit adaptation: memory-safe character copying */
